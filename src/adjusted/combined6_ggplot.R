...
packages <- function(x) {
  ...
  if (!require(x, character.only = TRUE)) {
    install.packages(pkgs = x, repos = "https://cran.rstudio.com/")
...
write.table(CSV_log[str:end,],file = "log10_region.csv",sep = "\t",quote = F,row.names = F)

N <- CSV_log[str:end,]

###
#N<-read.csv("./log10_region.csv", sep = "\t")
###
...
values = c("C"="deepskyblue2", "G"="black", 
           "A"="yellowgreen", "T"= "firebrick3")

ggraph4 <- function(N, a, b) {

RACE_plot<- ggplot(data = N, aes(x = seq_along(N[,1]))) +
...
# save_plot("seed.png", plot2_8, base_aspect_ratio = 1, base_height =12)

ggsave(filename="plot_seed.pdf", plot = plot2_8, scale = 1.8 )
ggsave(filename="plot_cleave.pdf", plot = plot9_12, scale = 0.8, width= 20.4 ,height = 7.8 )
