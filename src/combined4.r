...
ref_replace<- function(str,end, replicon_ref){
  ...
  for (i in nt_reference[str:end]){
    ...
    if (nt[str+a,1] == "A"){
      nt[,(ncol(nt)+1)]<- nt[,1]
      nt[str+a,ncol(nt)]<-"C"
      count_names[nrow(count_names)+1,]<-paste((str+a),nt[(str+a),1],"to","C", sep = "_" )
      nt[,(ncol(nt)+1)]<- nt[,1]
      nt[str+a,ncol(nt)]<-"T"
      count_names[nrow(count_names)+1,]<-paste((str+a),nt[(str+a),1],"to","T", sep = "_" )
      nt[,(ncol(nt)+1)]<- nt[,1]
      nt[str+a,ncol(nt)]<-"G"
      count_names[nrow(count_names)+1,]<-paste((str+a),nt[(str+a),1],"to","G", sep = "_" )
      a=a+1
    }
    if (nt[str+a,1] == "C"){
      nt[,(ncol(nt)+1)]<- nt[,1]
      nt[str+a,ncol(nt)]<-"A"
      count_names[nrow(count_names)+1,]<-paste((str+a),nt[(str+a),1],"to","A", sep = "_" )
      nt[,(ncol(nt)+1)]<- nt[,1]
      nt[str+a,ncol(nt)]<-"T"
      count_names[nrow(count_names)+1,]<-paste((str+a),nt[(str+a),1],"to","T", sep = "_" )
      nt[,(ncol(nt)+1)]<- nt[,1]
  ...
  for(i in (c(1:nrow(names_list)))){
    names_list[i,2] <- stri_flatten(trans_list[i,1:(ncol(trans_list))])
  }
  
  colnames(names_list)<- c("name", "target")
  names_list<-rbind((names_list[(nrow(names_list)),]), names_list)
